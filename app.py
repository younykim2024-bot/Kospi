...
# -----------------------------
# Files (relative paths are safest on Streamlit Cloud)
# -----------------------------
KOSPI200_CSV = os.environ.get('KOSPI200_CSV', 'kospi200.csv')
HISTORY_JSON = os.environ.get('KOSPI200_HISTORY_JSON', 'history.json')
...
# -----------------------------
# Price data fetch (prefer Naver/kr sources via FinanceDataReader; fallback to Yahoo via yfinance)
# -----------------------------
@st.cache_data(ttl=60*60, show_spinner=False)
def get_stock_data(code: str, start_year: int):
    """Fetch OHLCV DataFrame with columns Open/High/Low/Close/Volume and DatetimeIndex."""
    code = str(code).zfill(6)
    start = f"{int(start_year)}-01-01"
    end = datetime.datetime.now().strftime("%Y-%m-%d")

    # 1) FinanceDataReader (usually served from Naver/other KR sources, stable for KR tickers)
    if fdr is not None:
        try:
            df = fdr.DataReader(code, start, end)
            if df is not None and not df.empty:
                # FDR can return columns: Open High Low Close Volume Change
                df = df.copy()
                ...
                if all(c in df.columns for c in need):
                    if "Volume" not in df.columns:
                        # Some markets return 'Vol' etc
                        for alt in ["Vol","Volumes","Trading Volume"]:
    ...
    # 2) yfinance fallback (Yahoo) for .KS/.KQ
